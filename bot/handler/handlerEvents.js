...
module.exports = function (api, threadModel, userModel, dashBoardModel, globalModel, usersData, threadsData, dashBoardData, globalData) {
	return async function (event, message) {
		...
		let isUserCallCommand = false; // ✅ Flag for preventing double SMS
		...
		async function onStart() {
			...
			if (!command) {
				if (!hideNotiMessage.commandNotFound)
					await message.reply(
						commandName ? utils.getText({ lang: langCode, head: "handlerEvents" }, "commandNotFound", commandName, prefix) :
			...
			isUserCallCommand = true; // ✅ Command detected, set flag
		...
		async function onChat() {
			if (isUserCallCommand) return; // ✅ prevent double SMS
